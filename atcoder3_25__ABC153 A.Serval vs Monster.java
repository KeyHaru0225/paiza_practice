...
//	//Integer.parseInt(bin, 2);
	...
	static long LINF =(1L<<63)-1,mod=1000000000+7,count=0,sum=0,max=0,min=LINF,ans=0,temp;
	static int i=0,INF=(1<<31)-1;
	...
	static void fac(int N) {
		...
		System.out.println(root(b2)+" "+root);
		//=>root+root(b2)とroot-root(b2)を因数に持つ
		System.out.println((root+root(b2))+" "+(root-root(b2)));
		int a=root+root(b2);
		int b=root-root(b2);
	...
	private static int B(Map<Integer, ArrayList<Integer>> m,int DP[],int V) {
		if(!m.containsKey(V)||m.get(V).size()==0)return 0;
		if(DP[V]!=0)return DP[V];
		int res=0;
		for(int TO:m.get(V)) {
			res=max(res,B(m,DP,TO)+1);
		}
		return DP[V]=res;
	}
	private static void Topological_Sort(int V,Map<Integer,ArrayList<Integer>> m) {
		...
		while(!qq.isEmpty()) {
			...
			for (int i = 0; i < m.get(v).size(); i++) {
				int u=m.get(v).get(i);
				indegree[u]--;
				if(indegree[u]==0)qq.add(u);
			}
		}
		
	}
	static void check(int a[],int A[],int B[],int C[],int D[]) {
		temp=0;
	...
	static int isBRO(String a,String b,String c) {
		char A=a.charAt(0),A2=a.charAt(a.length()-1);
		char B=b.charAt(0),B2=b.charAt(b.length()-1);
		char C=c.charAt(0),C2=c.charAt(c.length()-1);
		if((A==B2||A==B)&&(A==C||A==C2)) {
			return A;
		}
		if((A2==B2||A2==B)&&(A2==C||A2==C2)) {
			return A2;
		}
		return -1;
	...
	//外から呼び出すときはi==-1
	static long DFS(int v,int i,Map<Integer, ArrayList<V>> m,long DP[][]) {
		//vから出るi番目の辺の部分木の値を返す。
		if(!m.containsKey(v)||m.get(v).size()==0)return 0;//先に頂点が無いならば、そこで終わり
		long res=0;
		int t=0;
		for(V edge:m.get(v)) {
			//mから出ているedgeを探索する。
